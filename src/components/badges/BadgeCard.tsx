      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-sm font-bold text-ink-900">{tx(badge.title)}</h3>
            {!badge.earned && <Lock size={12} className="shrink-0 text-slate-400" />}
          </div>
          <p className="mt-1 text-xs leading-relaxed text-slate-500">{tx(badge.description)}</p>
        </div>
      </div>
      ...
      <div className="mt-3.5 border-t border-slate-200/70 pt-3">
        {badge.earned ? (
          <p className="text-[11px] font-bold text-green-600">
            {t('badges.earnedAt', { when: badge.earnedAt ? tx(badge.earnedAt) : '' })}
          </p>
        ) : (
